...
from model import BetaVAE_H, BetaVAE_B, Discriminator
...
class Solver(object):
    def __init__(self, args):
        ...
        # Discriminator
        self.discriminator = Discriminator(in_channels=3, features=[64, 128, 256, 512], norm_type=self.norm_type).to('cuda')
        self.opt_disc = optim.Adam(self.discriminator.parameters(), lr=self.lr, betas=(self.beta1, self.beta2))
        ...
        self.data_loader_1 = return_data(args)
    ...
    def train(self):
        ...
        while not out:
            ...
            # for x1 in self.data_loader_1:
                print(x1)
                x_recon, mu, logvar = self.net(x)
                x_recon_1,mu,logavar = self.net(x) # what is logavar?

                viz = visdom.Visdom()
                x_recon = F.sigmoid(x_recon)
                x_recon_1 = F.sigmoid(x_recon_1)

                viz.images(x, nrow=8, win='x', opts=dict(title='x'))
                viz.images(x_recon, nrow=8, win='x_recon', opts=dict(title='x_recon'))
                viz.images(x1, nrow=8, win='x1', opts=dict(title='x1'))
                viz.images(x_recon_1, nrow=8, win='x_recon_1', opts=dict(title='x_recon_1'))
                ...
                loss_D = self.lambda_D * self.loss_D(disc_real, disc_fake)
                ...
                disc_fake = self.discriminator(x_recon)
                loss_G = self.lambda_G * self.loss_G(disc_fake)
                loss_PCP = self.lambda_PCP * self.loss_PCP(x_recon, x)
                beta_vae_loss = beta_vae_loss + loss_G + loss_PCP
                ...
                if self.global_iter%self.display_step == 0:
                    pbar.write('[{}] beta_vae_loss:{:.3f} recon_loss:{:.3f} total_kld:{:.3f} mean_kld:{:.3f} loss_D:{:.3f} loss_G:{:.3f} loss_PCP:{:.3f} '.format(
                        self.global_iter, beta_vae_loss, recon_loss.item(), total_kld.item(), mean_kld.item(), loss_D, loss_G, loss_PCP))
        ...
        pbar.close()
        # torch.save(BetaVAE_H, 'BetaVAE_H_0323.pth')
        ...
        if self.save_output:
            ...
            for i, key in enumerate(Z.keys()):
                for j, val in enumerate(interpolation):
                    self.save_image(tensor=gifs[i][j].cpu(),
    ...
    def save_image(self, tensor, filename, nrow=8, padding=2,
        ...
        # viz = visdom.Visdom()
        # viz.images(ndarr, nrow=8, win='ndarr', opts=dict(title='ndarr'))
